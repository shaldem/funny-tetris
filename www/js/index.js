...
function  onDeviceReady (id) {
	...
	var ctxSz = Math.min(window.innerHeight, window.innerWidth, 400);
	ctxSz -= ctxSz%step;

	cnt.canvas.height = ctxSz-step;
	cnt.canvas.width = ctxSz-step;

	var fWlen = cnt.canvas.width/step;
	var fHlen = cnt.canvas.height/step;
		...
		boardB: function (x,y) {
			...
			if(!dnBotRed) {
				for(var i=0; i<w; i++) {
					...
					if(figure.flist[figure.curr.shape].pict[figure.curr.state][i][h-1] == 0) {
						var Hb =  figure.getHi(i);
					}

					if(y>=0 && fArr[x+i][y+Hb-1] == 1){
						dnBotRed = true;
						break;
					}
				}
